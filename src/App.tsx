...
function urlFromPage(page: Page): string {
  if (page === "home") return "/";
  ...
  if (page.startsWith("post-")) {
    return `/post/${encodeURIComponent(page.replace("post-", ""))}`;
  }
  return "/";
}
...
export default function App() {
  ...
  // 페이지 제목/설명 동적 업데이트 (SEO)
  useEffect(() => {
    const baseSite = "하우징허브";
    let title = `${baseSite} - 대한민국 최고의 부동산 & 주거 정보 가이드`;
    let description = "청약 정보, 전월세 계약 팁, 담보대출 가이드 등 실용적인 주거 정보를 제공하는 하우징허브입니다.";

    if (currentPost) {
      title = `${currentPost.title} | ${baseSite}`;
      description = currentPost.excerpt;
    } else if (currentPage === "about") {
      title = `소개 | ${baseSite}`;
    } else if (currentPage === "privacy") {
      title = `개인정보 처리방침 | ${baseSite}`;
    } else if (currentPage.startsWith("category-")) {
      const cat = currentPage.replace("category-", "");
      title = `${cat} 정보 | ${baseSite}`;
    }
